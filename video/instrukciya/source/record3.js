...
const OUT = LANG === 'ru' ? `rec2_${DEV}` : `rec3_${LANG}_${DEV}`;
const LBL = {
  ru: {menu: /Подарить сертификат/, accept: /Принимаю/, next: /^\s*Далее/, occ: 'День рождения', occ0: 'Без повода', prog: 'Ты и Я', price: '70 000 ₸', tabSum: 'На сумму', sumPc: '50 000 ₸', sumMob: '70 000', greet: /Добавить поздравление/, city: 'Алматы', salonPc: 'Имбирь в ЖК «Шанырак»', salonMob: /Шанырак/, nowPc: 'Сразу после оплаты', nowMob: /^\s*Сразу\s*$/, datePc: 'В выбранную дату', dateMob: /^\s*Выбрать дату\s*$/, total: 'Итого', promo: /Промокод/, card: 'Карта', pay: /Оплатить/, n1: 'Айгерим', n2: 'Данияр', msg: 'С днём рождения! Пусть этот день будет тёплым и спокойным.'},
  kk: {menu: /Сертификат сыйлау/i, accept: /Қабылдаймын/, next: /^\s*Әрі қарай/, occ: 'Туған күн', occ0: 'Жай ғана', prog: 'Сен және Мен', price: '70 000 ₸', tabSum: 'Сомаға', sumPc: '50 000 ₸', sumMob: '70[ ,\u00a0]?000', greet: /Құттықтау қосу/, city: 'Алматы', salonPc: 'Imbir — «Шаңырақ» ТҮК', salonMob: /Шаңырақ/, nowPc: 'Төлемнен кейін бірден', nowMob: /^\s*Бірден\s*$/, datePc: 'Таңдалған күні', dateMob: /^\s*Күнін таңдау\s*$/, total: 'Барлығы', promo: /Промокод/, card: 'Карта', pay: /Төлеу/, n1: 'Айгерім', n2: 'Данияр', msg: 'Туған күніңмен! Бұл күн жылы әрі тыныш өтсін.'},
  en: {menu: /Gift a certificate/i, accept: /^\s*Accept\s*$/, next: /^\s*Next/, occ: 'Birthday', occ0: 'Just because', prog: 'You & I', price: '70 000 ₸', tabSum: 'Amount', sumPc: '50 000 ₸', sumMob: '70[ ,\u00a0]?000', greet: /Add a message/, city: 'Almaty', salonPc: 'Imbir — Shanyrak residence', salonMob: /Shanyrak/, nowPc: 'Right after payment', nowMob: /^\s*Right away\s*$/, datePc: 'On a chosen date', dateMob: /^\s*Pick a date\s*$/, total: 'Total', promo: /Promo code/, card: 'Card', pay: /^\s*Pay/, n1: 'Aigerim', n2: 'Daniyar', msg: 'Happy birthday! Wishing you a warm and peaceful day.'},
}[LANG]; const FPS = 30, DT = 1000 / FPS;
const LINES = JSON.parse(fs.readFileSync(LANG === 'ru' ? 'lines.json' : `lines_${LANG}.json`, 'utf8'))[DEV];
...
(async () => {
  ...
  await settle(`https://www.imbir.kz/${LANG}`);
  ...
  const A = {
    ...
    home: async () => {
      ...
      else { await wait(900); await click(p.locator('header button:visible').last(), {ms: 700, after: 900}); await click(p.getByRole('link', {name: LBL.menu}).locator('visible=true').first(), {ms: 600, after: 150}); }
      await navigated();
    },
    consent_read: async () => {
    ...
    consent_accept: async () => {
      ...
      await click(cbl, {ms: 800, after: 500}); await click(vbtn(LBL.accept), {ms: 700, after: 300});
      if (PC) await cam(null); await wait(700);
    },
    occasion: async () => { if (PC) await click(T(LBL.occ), {ms: 900, after: 700}); else { await arc(LBL.occ0, 590, 3); await arc(LBL.occ, 590, 6); } },
    designs: async () => {
      if (LANG === 'kk') {   // browse forward, then come back to the Kazakh-language card (first in the list)
        if (PC) { await click(T('›'), {ms: 800, after: 800}); await click(T('‹'), {ms: 700, after: 800}); }
        else { await swipe(250, 705, 130, 705, 520); await wait(900); await swipe(130, 705, 250, 705, 520); await wait(700); }
      } else if (PC) { const r = T('›'); await click(r, {ms: 800, after: 800}); await click(r, {ms: 250, after: 800}); }
      else { await swipe(250, 705, 130, 705, 520); await wait(900); await swipe(250, 705, 130, 705, 520); await wait(700); }
    },
    next: async () => { await click(vbtn(LBL.next), {ms: 700, after: 900}); },
    program: async () => {
      if (PC) { await click(T(LBL.prog), {ms: 900, after: 400}); await cam(T(LBL.price).locator('xpath=../..'), 60); await hover(T(LBL.price), 700).catch(() => {}); await wait(1300); await cam(null); }
      else { await swipe(290, 690, 110, 690, 650); await wait(700); await arc(LBL.prog.replace('&', '\\&'), 690); await wait(800); }
    },
    amount: async () => { await click(T(LBL.tabSum), {ms: 800, after: 800}); if (PC) await click(T(LBL.sumPc, false), {ms: 900, after: 700}); else await arc(LBL.sumMob, 660, 6); },
    name: async () => { const i = inputs().nth(0); if (PC) await cam(p.locator('form:visible, .stg__content:visible').first(), 30).catch(() => {}); await click(i, {ms: 800, after: 150}); await type(LBL.n1, 4); },
    from: async () => { await click(inputs().nth(1), {ms: 600, after: 150}); await type(LBL.n2, 4); },
    greeting: async () => { await click(vbtn(LBL.greet), {ms: 700, after: 500}); await click(p.locator('textarea:visible').first(), {ms: 500, after: 150}); await type(LBL.msg, 2); },
    branch: async () => {
      if (PC) await cam(null);
      if (PC) { await click(T(LBL.city), {ms: 800, after: 800}); await click(T(LBL.salonPc), {ms: 800, after: 400}); }
      else { await arc(LBL.city, 660, 5); await click(vbtn(LBL.salonMob), {ms: 500, after: 400}); }
    },
    when_now: async () => { await wait(1500); const bb = await hover(PC ? T(LBL.nowPc) : vbtn(LBL.nowMob), 800); ring(bb); },
    when_date: async () => { await click(PC ? T(LBL.datePc) : vbtn(LBL.dateMob), {ms: 600, after: 1400}); },
    email_buyer: async () => {
      await click(PC ? T(LBL.nowPc) : vbtn(LBL.nowMob), {ms: 500, after: 400});
    ...
    summary: async () => { if (PC) await cam(null); await wait(400); const bb = await box(T(LBL.total, false)).catch(() => null); if (bb && PC) await moveTo(bb.x + 260, bb.y - 90, 900); await wait(800); },
    promo: async () => { await click(vbtn(LBL.promo), {ms: 800, after: 500}); const pi = p.locator('input[placeholder]:visible').last(); if (PC) await cam(pi.locator('xpath=../..'), 120).catch(() => {}); await click(pi, {ms: 500, after: 800}).catch(() => {}); },
    paymethod: async () => { if (PC) await cam(T(LBL.card, false).locator('xpath=../..'), 120).catch(() => {}); await click(PC ? T(LBL.card, false) : vbtn(new RegExp('^\\s*' + LBL.card)), {ms: 800, after: 1000}); await click(PC ? T('Kaspi.kz', false) : vbtn(/Kaspi/), {ms: 600, after: 600}); },
    pay: async () => {
      ...
      await wait(500); const pb = vbtn(LBL.pay); const bb = await hover(pb, 800); ring(bb); await wait(900);
    },
  };
