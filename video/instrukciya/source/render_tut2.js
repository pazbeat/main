...
const DEV = process.argv[2], WK = +process.argv[3] || 4; const PC = DEV === 'pc';
(async () => {
  const b = await chromium.launch();
  const vp = PC ? {width: 1920, height: 1080} : {width: 1080, height: 1920};
  const probe = await b.newPage({viewport: vp}); await probe.goto(`http://127.0.0.1:8123/tut/tut2.html?dev=${DEV}`); const TOTAL = await probe.evaluate(() => window.ready); await probe.close();
  ...
  await Promise.all([...Array(WK).keys()].map(async w => {
    ...
    await p.goto(`http://127.0.0.1:8123/tut/tut2.html?dev=${DEV}`); await p.evaluate(() => window.ready);
    const ff = spawn(FF, ['-loglevel', 'error', '-y', '-f', 'image2pipe', '-framerate', '30', '-c:v', 'mjpeg', '-i', '-', '-c:v', 'libx264', '-preset', 'medium', '-crf', '18', '-pix_fmt', 'yuv420p', '-r', '30', `seg2_${DEV}_${w}.mp4`], {stdio: ['pipe', 'inherit', 'inherit']});
