...
const FPS=30, DUR=76, TOTAL=FPS*DUR;
...
(async () => {
  ...
  await Promise.all([...Array(W).keys()].map(async w => {
    ...
    await p.goto('file://'+__dirname+'/promo.html');
    const ok = await p.evaluate(()=>window.ready); if(!ok) console.log('fonts not ok', w);
    const ff = spawn(FF,['-loglevel','error','-y','-f','image2pipe','-framerate',String(FPS),'-c:v','mjpeg','-i','-','-c:v','libx264','-preset','medium','-crf','17','-pix_fmt','yuv420p','-r',String(FPS),`seg_${w}.mp4`],{stdio:['pipe','inherit','inherit']});
    for (let f=f0; f<f1; f++) {
